...
class Subject:
    def __init__(self, subject_id, subject_name, credit: int):
...
def search_enroll_by_student(student):
    result = []
...
def enroll_to_subject(student: Student, subject: Subject):
    ...
    if search_enrollment_subject_student(subject, student) != "Not found":
...
def drop_from_subject(student: Student, subject: Subject):
    ...
    already_exist_enrollment = search_enrollment_subject_student(subject, student)

    if already_exist_enrollment != "Not found":
        enrollment_list.remove(already_exist_enrollment)
        return "Dropped"
    else:
        return "Not Found"
...
def search_enrollment_subject_student(subject: Subject, student: Student):
    student_enrollment = search_enroll_by_student(student)

    for this_enrollment in student_enrollment:
        if this_enrollment.subject == subject:
            return this_enrollment
...
def assign_grade(student: Student, subject: Subject, grade: str):
    ...
    enrollment.assign_grade(grade_to_count(grade))
    return "Done"
...
def get_student_record(student):
    ...
    for enrollment in search_subject_that_student_enrolled(student):
        result[enrollment.subject.subject_id] = [
            enrollment.subject.subject_name,
            count_to_grade(enrollment.grade),
        ]
...
def count_to_grade(count):
    grade_mapping = {4: "A", 3: "B", 2: "C", 1: "D"}
    return grade_mapping.get(count, "F")


# TODO 12 : function สำหรับคำนวณเกรดเฉลี่ยของนักศึกษา โดยรับ instance ของ student
def get_student_GPS(student):

    all_enrollment = search_subject_that_student_enrolled(student)

    sum_score = 0
    sum_credit = 0

    for enrollment in all_enrollment:
        sum_score += enrollment.grade * enrollment.subject.credit
        sum_credit += enrollment.subject.credit
...
print(
    "Answer : {'66010001': 'Keanu Welsh', '66010002': 'Khadijah Burton', '66010003': 'Jean Caldwell', '66010004': 'Jayden Mccall', '66010005': 'Owain Johnston', '66010007': 'Frances Haynes'}"
)
print(student_enroll)
print(student_enroll["66010005"])
print("")
...
print(
    "Answer : {'66010002': 'Khadijah Burton', '66010003': 'Jean Caldwell', '66010004': 'Jayden Mccall', '66010005': 'Owain Johnston', '66010007': 'Frances Haynes'}"
)
...
print(
    "Answer : {'CS101': ['Computer Programming 1', 'A'], 'CS102': ['Computer Programming 2', 'B'], 'CS103': ['Data Structure', 'C']}"
)
